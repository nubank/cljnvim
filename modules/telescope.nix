{
  ...
  cfg = config.cljnvim.telescope;
in {
  options.cljnvim.telescope.enable = mkEnableOption "Enables Telescope GUI";
  ...
  config.cljnvim = mkIf cfg.enable {
    ...
    nnoremap =
      ...
      // (
        withAttrSet config.cljnvim.lsp.enable {
      ...
      // (
        withAttrSet config.cljnvim.treesitter.enable {
